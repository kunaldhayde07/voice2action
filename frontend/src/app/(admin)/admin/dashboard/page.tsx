...
export default function AdminDashboardPage() {
  ...
  const STAT_CARDS = [
    ...
    },
    {
      ...
      trend: { value: 3, label: 'vs last month', direction: 'down' as const },
    },
    {
      title: 'In Progress',
    ...
    },
    {
      ...
      trend: { value: 24, label: 'new this month', direction: 'up' as const },
    },
    {
      title: 'Total Votes',
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        icon={BarChart3}
        badge={{ label: 'Live', color: 'bg-green-100 text-green-700' }}
        actions={
          <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 civic-card p-5">
          ...
          <TrendChart
            data={dashboardData?.monthlyTrend?.map((m) => ({
              _id: `${m._id.year}-${String(m._id.month).padStart(2, '0')}-01`,
              count: m.count,
              resolved: m.resolved,
            })) ?? []}
            isLoading={isLoading}
          ...
          {isLoading ? (
            ...
            <div className="space-y-3">
              {dashboardData?.recentIssues?.map((issue) => (
                <IssueCard key={issue._id} issue={issue} compact />
              ))}
            </div>
          )}
        </div>
